...
def query(q):
    ...
    Gracefully handle missing tables or empty result sets so that
    views don't crash with OperationalError or IndexError.
    """
    try:
        ...
        # Database not fully initialized (e.g. tables not created yet).
        return [], []
    # When there are no rows, just return the column names (if any).
    keys = columns
    return data, keys
